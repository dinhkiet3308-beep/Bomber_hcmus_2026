...
class TacticalRuleAgent:
    ...
    def act(self, obs):
        ...
        phase = "early" if self.step_count <= 120 else ("mid" if self.step_count <= 275 else "late")
        ...
        # ============================================================
        # PRIORITY 2: PLACE BOMB (valuable targets + pressure/stats)
        # ============================================================
        if int(bombs_left) > 0 and my_pos not in bomb_positions:
            bomb_action = self._consider_bombing(
                grid, my_pos, enemies, bomb_radius, blocked, danger_times, phase, enemy_set, box_spots, int(bombs_left), obs
            )
        ...
        # (NOTE: Old Priority 5 STAT BOMB has been deleted and merged into Priority 2)

        # ============================================================
        # PRIORITY 5: STRATEGIC FALLBACK (Formerly Priority 6)
        # ============================================================
    ...
    def _move_to_targets(self, grid, start, targets, blocked, danger_times, enemy_set, max_dist=15):
        ...
        while pq:
            ...
            for a in (1, 2, 3, 4):
                ...
                # Danger check: Space-time awareness
                if self._is_tile_dangerous_at(danger_times, npos, next_dist):
                    continue
                # Don't step onto a tile if it explodes the turn AFTER we arrive 
                # (unless it is our final destination target)
                if npos not in targets and self._is_tile_dangerous_at(danger_times, npos, next_dist + 1):
                    continue
    ...
    def _consider_bombing(self, grid, my_pos, enemies, bomb_radius, blocked, danger_times, phase, enemy_set, box_spots, bombs_left, obs):
        """
        ...
        - NEW: Pressure & Tie-breaker overrides!
        """
        my_blast = self._blast_tiles(grid, my_pos[0], my_pos[1], bomb_radius)

        enemy_hits = sum(1 for ex, ey in enemies if (ex, ey) in my_blast)
        ...
        # ==========================================================
        # THE "ANTI-ZOMBIE" & TIE-BREAKER PATCH
        # ==========================================================
        my_exits = self._open_neighbors(grid, my_pos, blocked)
        closest_enemy_dist = min([self._manhattan(my_pos, e) for e in enemies]) if enemies else 999
        is_board_empty = not box_spots

        if score < 1:
            # 1. Anti-Zombie Pressure: Drop bomb if enemy is very close, forcing them to reroute
            if closest_enemy_dist <= 3 and my_exits >= 2:
                score = 1
                
            # 2. Tie-Breaker Farming: If board is empty or late game, spam bombs safely
            elif (self.step_count >= 200 or is_board_empty) and my_exits >= 3:
                # Don't blow our last bomb just for stats if there are enemies around
                if bombs_left > 1 or closest_enemy_dist > 6:
                    score = 1

        if score < 1:
            return None

        # Can we escape after placing? (Now correctly passing obs)
        if not self._can_escape_after_placing(grid, my_pos, blocked, danger_times, bomb_radius, enemy_set, obs):
            return None

        # Dead-end check: never trap yourself for a low-value bomb
        if my_exits <= 1 and enemy_hits == 0:
            return None
